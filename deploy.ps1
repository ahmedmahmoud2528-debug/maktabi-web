...
npm run build
$nojekyll = Join-Path $PSScriptRoot "dist\.nojekyll"
if (-not (Test-Path $nojekyll)) { New-Item -ItemType File -Path $nojekyll | Out-Null }
[IO.File]::WriteAllText((Join-Path $PSScriptRoot "dist\version.txt"), [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds().ToString())
